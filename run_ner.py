...
#         strategy = tf.distribute.OneDeviceStrategy(device=f"/gpu:{gpu}")
    ...
    if args.do_train:
        ...
        train_data = tf.data.Dataset.zip(
            (all_input_ids, all_input_mask, all_segment_ids, all_valid_ids,all_label_mask,all_label_ids))
        shuffled_train_data = train_data.shuffle(buffer_size=int(len(train_features) * 1.0),
                                                seed = args.seed,
        ...
        def train_step(input_ids, input_mask, segment_ids, valid_ids, label_ids,label_mask):
            ...
            per_example_losses = strategy.run(
                step_fn,args=(input_ids, input_mask, segment_ids, valid_ids, label_ids,label_mask))
